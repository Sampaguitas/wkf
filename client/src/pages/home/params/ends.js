...
export default class Ends extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            _id: "",
            ...
            params: {
                ...
                end_name: { value: "", placeholder: "Name", selection: { _id: "", name: ""}, options: [], hover: "", page: 0 },
                end_lunar: { value: "", placeholder: "vLunar", selection: { _id: "", name: ""}, options: [], hover: "", page: 0 },
                end_tags: { value: "", placeholder: "Tags", selection: { _id: "", name: ""}, options: [], hover: "", page: 0, selectionArray: [] },
                end_pffTypes: { value: "", placeholder: "PFF Types", selection: { _id: "", name: ""}, options: [], hover: "", page: 0, selectionArray: [] },
            },
            focused: "",
            alert: {
                type: "",
    ...
    componentDidUpdate(prevProps, prevState) {
        ...
        if (this.state.params.end_tags.value !== prevState.params.end_tags.value) this.getDropdownOptions("end_tags", 0);
        if (this.state.params.end_pffTypes.value !== prevState.params.end_pffTypes.value) this.getDropdownOptions("end_pffTypes", 0);
    ...
    getDocuments(nextPage) {
        ...
        if (!!paginate.pageSize) {
            this.setState({
                ...
                return fetch(`${process.env.REACT_APP_API_URI}/server/ends/getAll`, requestOptions)
        ...
        if (!upserting) {
            this.setState({
                ...
                const requestOptions = {
                    ...
                    headers: { ...authHeader(), "Content-Type": "application/json" },
                    body: JSON.stringify({
                        name: params.end_name.selection._id,
                        lunar: params.end_lunar.selection._id,
                        tags: params.end_tags.selectionArray,
                        pffTypes: params.end_pffTypes.selectionArray,
                    })
                };
                return fetch(`${process.env.REACT_APP_API_URI}/server/ends/${!!this.state._id ? this.state._id : ""}`, requestOptions)
                .then(response => response.text().then(text => {
                    this.setState({
                        upserting: false,
                    }, () => {
    ...
    handleDelete(event) {
        ...
        if (!!_id && !deleting && !!window.confirm("Would you like to permanantly delete the entry?")) {
            this.setState({
                ...
                return fetch(`${process.env.REACT_APP_API_URI}/server/ends/${_id}`, requestOptions)
                .then(response => response.text().then(text => {
                    this.setState({
                        deleting: false,
                    }, () => {
        ...
        if (!!currentUser.isAdmin) {
            this.setState({
                ...
                params: {
                    ...
                    end_name: { value: "", placeholder: "Name", selection: { _id: "", name: ""}, options: [], hover: "", page: 0 },
                    end_lunar: { value: "", placeholder: "vLunar", selection: { _id: "", name: ""}, options: [], hover: "", page: 0 },
                    end_tags: { value: "", placeholder: "Tags", selection: { _id: "", tags: ""}, options: [], hover: "", page: 0, selectionArray: [] },
                    end_pffTypes: { value: "", placeholder: "PFF Types", selection: { _id: "", name: ""}, options: [], hover: "", page: 0, selectionArray: [] },
                },
                ...
                .then(response => response.text().then(text => {
                    ...
                    if (response.status === 401) {
                        ...
                        this.setState({
                            params: {
                                ...this.state.params,
                                end_name: { value: data.doc.name || "", placeholder: "Name", selection: { _id: data.doc.name || "", name: data.doc.name || ""}, options: [], hover: "", page: 0 },
                                end_lunar: { value: data.doc.lunar, placeholder: "vLunar", selection: { _id: data.doc.lunar, name: data.doc.lunar}, options: [], hover: "", page: 0 },
                                end_tags: { value: "", placeholder: "Tags", selection: { _id: "", tags: ""}, options: [], hover: "", page: 0, selectionArray: [...data.doc.tags] },
                                end_pffTypes: { value: "", placeholder: "PFF Types", selection: { _id: "", name: ""}, options: [], hover: "", page: 0, selectionArray: [...data.doc.pffTypes] },
                            },
                            retrievingElement: false,
                        });
                    }
    ...
    getDropdownOptions(key, page) {
        ...
        this.setState({
            ...
            return fetch(`${process.env.REACT_APP_API_URI}/server/ends/getDrop/${key}`, requestOptions)
            .then(response => response.text().then(text => {
                this.setState({
                    loading: false,
                }, () => {
            ...
            <Layout collapsed={collapsed} toggleCollapse={toggleCollapse} menuItem={menuItem}>
                ...
                <div id="export" className={alert.message ? "main-section-alert" : "main-section"}>
                    <div className="action-row row">
                        ...
                        <button title="Create End" className="btn btn-sm btn-gray" onClick={this.toggleModalSubmit} disabled={!currentUser.isAdmin ? true : false}> {/* style={{height: "34px"}} */}
                            <span><FontAwesomeIcon icon="plus" className="fa mr-2" />Create End</span>
                        </button>
                    </div>
                                        ...
                                        <section id="singles" className="drop-section">
                                            ...
                                            <div className="row row-cols-1 row-cols-md-2">
                                                <ParamInput
                                                    ...
                                                    name="end_name"
                                                    focused={focused}
                                                    value={params.end_name.selection.name}
                                                    placeholder={params.end_name.placeholder}
                                                ...
                                                <ParamInput
                                                    ...
                                                    name="end_lunar"
                                                    focused={focused}
                                                    value={params.end_lunar.selection.name}
                                                    placeholder={params.end_lunar.placeholder}
                                                    onChange={this.handleChangeInput}
                                                    onFocus={this.onFocus}
                                                    handleClearValue={this.handleClearValue}
                                                />
