...
export default class Pffs extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            _id: "",
            ...
            params: {
                ...
                pff_name: { value: "", placeholder: "Name", selection: { _id: "", name: ""}, options: [], hover: "", page: 0 }
            },
            focused: "",
            alert: {
                type: "",
    ...
    getDocuments(nextPage) {
        ...
        if (!!paginate.pageSize) {
            this.setState({
                ...
                return fetch(`${process.env.REACT_APP_API_URI}/server/pffs/getAll`, requestOptions)
        ...
        if (!upserting) {
            this.setState({
                ...
                const requestOptions = {
                    ...
                    headers: { ...authHeader(), "Content-Type": "application/json" },
                    body: JSON.stringify({
                        name: params.pff_name.selection._id,
                    })
                };
                return fetch(`${process.env.REACT_APP_API_URI}/server/pffs/${!!this.state._id ? this.state._id : ""}`, requestOptions)
                .then(response => response.text().then(text => {
                    this.setState({
                        upserting: false,
                    }, () => {
    ...
    handleDelete(event) {
        ...
        if (!!_id && !deleting && !!window.confirm("Would you like to permanantly delete the entry?")) {
            this.setState({
                ...
                return fetch(`${process.env.REACT_APP_API_URI}/server/pffs/${_id}`, requestOptions)
                .then(response => response.text().then(text => {
                    this.setState({
                        deleting: false,
                    }, () => {
    ...
    handleOnclick(event, _id) {
        ...
        if (!_.isUndefined(found) && !!currentUser.isAdmin) {
            this.setState({
                ...
                params: {
                    ...this.state.params,
                    pff_name: { value: found.name, placeholder: "Name", selection: { _id: found.name, name: found.name}, options: [], hover: "", page: 0 },
                },
                showSubmit: true
            });
        }
    ...
    //                 headers: { ...authHeader(), "Content-Type": "application/json" },
    //             };
    //             return fetch(`${process.env.REACT_APP_API_URI}/server/pffs/download/${exportId}`, requestOptions)
    //             .then(response => {
    ...
    getDropdownOptions(key, page) {
        ...
        this.setState({
            ...
            return fetch(`${process.env.REACT_APP_API_URI}/server/pffs/getDrop/${key}`, requestOptions)
            .then(response => response.text().then(text => {
                this.setState({
                    loading: false,
                }, () => {
            ...
            <Layout collapsed={collapsed} toggleCollapse={toggleCollapse} menuItem={menuItem}>
                ...
                <div id="export" className={alert.message ? "main-section-alert" : "main-section"}>
                    <div className="action-row row">
                        ...
                        <button title="Create PFF Type" className="btn btn-sm btn-gray" onClick={this.toggleModalSubmit} disabled={!currentUser.isAdmin ? true : false}> {/* style={{height: "34px"}} */}
                            <span><FontAwesomeIcon icon="plus" className="fa mr-2" />Create PFF</span>
                        </button>
                    </div>
                                ...
                                <section id="filters" className="drop-section">
                                    <div className="row row-cols-1">
                                        <ParamInput
                                            ...
                                            name="pff_name"
                                            focused={focused}
                                            value={params.pff_name.selection.name}
                                            placeholder={params.pff_name.placeholder}
